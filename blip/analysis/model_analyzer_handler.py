...
class ModelAnalyzerHandler:
    ...
    def load_model_analyzer_function(self,
    ...
    def process_config(self):
        ...
        if "custom_model_analyzer_file" in self.config.keys():
            if os.path.isfile(self.config["custom_model_analyzer_file"]):
                try:
                    ...
                    self.logger.info(f'added custom model analyzer function from file {self.config["custom_model_analyzer_file"]}.')
                except:
                    self.logger.error(
                        f'loading classes from file {self.config["custom_model_analyzer_file"]} failed!'
                    )
            else:
                self.logger.error(f'custom_model_analyzer_file {self.config["custom_model_analyzer_file"]} not found!')
        ...
        for item in self.config.keys():
            ...
            if item in self.model_analyzers:
                self.logger.warn(f'duplicate model analyzer specified in config! Attempting to arrange by target_type.')
                if self.model_analyzers[item].target_type == self.config[item]['target_type']:
                    self.logger.error(f'duplicate model analyzeres with the same target_type in config!')
