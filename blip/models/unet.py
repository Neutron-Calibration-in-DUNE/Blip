...
class DoubleConv2DLayer(GenericModel):
    def __init__(self,
        ...
        if activation not in activations.keys():
            self.logger.error(f"specified activation '{self.cfg['activation']} not in allowed list ({activations.keys()})!")
...
class UNet(GenericModel):
    ...
    def __init__(self,
        name:   str,
        cfg:    dict=unet_config   # configuration parameters
    ):
    ...
    def construct_model(self):
        ...
        for filter in self.cfg['filtrations']:
            _down_dict[f'down_filter_double_conv{filter}'] = DoubleConv2DLayer(
                ...
                padding=self.cfg['double_conv_padding'],
        ...
        for filter in reversed(self.cfg['filtrations']):
            _up_dict[f'up_filter_transpose{filter}'] = nn.ConvTranspose2d(
                ...
                padding=self.cfg['conv_transpose_padding']    
            )
        ...
        self.output = nn.Conv2d(
            self.cfg['filtrations'][0],    # to match first filtration
            self.cfg['out_channels'],      # to the number of classes
