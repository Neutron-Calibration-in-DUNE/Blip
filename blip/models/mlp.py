...
class MLP(GenericModel):
    ...
    def construct_model(self):
        ...
        if "layers" not in self.config.keys():
            self.logger.error(f'layers not specified for MLP config!')
        ...
        if "dropout" not in self.config.keys():
            self.logger.warn(f'dropout not specified in config! setting to 0.')
            ...
            if isinstance(self.config['dropout'], float):
                ...
                if len(self.config['dropout']) != len(self.config['layers']) or len(self.config['dropout']) != len(self.config['layers'] - 1):
                    self.logger.error(f'specified dropout values {self.config["dropout"]} not equal to the number of layers, or the number of layers - 1!')
            else:
        ...
        if "activation" not in self.config.keys():
            self.logger.warn(f'activation not specified in config! setting to "leaky_relu".')
            ...
            if self.config['activation'] in activations.keys():
                if 'activation_params' in self.config.keys():
                    try:
                        ...
                        self.logger.error(f'setting activation in MLP failed with inputs {self.config["activation"]},{self.config["activation_params"]}!')
                else:
                    try:
                        self.activation = activations[self.config['activation']]()
                    except:
                        self.logger.error(f'setting activation in MLP failed with inputs {self.config["activation"]}!')
        ...
        # record the info
        self.logger.info(f"Constructed MLP with dict: {self.mlp_dict}.")
