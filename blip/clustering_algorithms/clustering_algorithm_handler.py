...
class ClusteringAlgorithmHandler:
    ...
    def __init__(self,
        ...
        if bool(config) and len(clustering_algorithms) != 0:
            self.logger.error(
                f"handler received both a config and a list of clustering_algorithms! " + 
                f"The user should only provide one or the other!")
        elif bool(config):
            self.set_config(config)
        else:
            if len(clustering_algorithms) == 0:
                self.logger.error(f"handler received neither a config or clustering_algorithms!")
            self.clustering_algorithms = {
    ...
    def load_clustering_algorithm_function(self,
        criterion_file: str
    ):
        ...
        if "custom_clustering_algorithm_file" in self.config.keys():
            if os.path.isfile(self.config["custom_clustering_algorithm_file"]):
                try:
                    ...
                    self.logger.info(f'added custom clustering_algorithm function from file {self.config["custom_clustering_algorithm_file"]}.')
                except:
                    self.logger.error(
                        f'loading classes from file {self.config["custom_clustering_algorithm_file"]} failed!'
                    )
            else:
                self.logger.error(f'custom_clustering_algorithm_file {self.config["custom_clustering_algorithm_file"]} not found!')
    ...
    def add_clustering_algorithm(self,
        clustering_algorithm:   GenericClusteringAlgorithm
    ):
        if issubclass(clustering_algorithm, GenericClusteringAlgorithm):
            self.logger.info(f'added clustering_algorithm function "{clustering_algorithm}" to Clusteringclustering_AlgorithmHandler.')
            ...
            self.logger.error(
                ...
                f' Only clustering_algorithm functions which inherit from GenericClusteringAlgorithm can' +
                f' be used by the ClusteringAlgorithmHandler in BLIP.'
            )
