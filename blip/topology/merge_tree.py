...
class MergeTree:
    ...
    def create_merge_tree(self,
        ...
        if point_cloud is None:
            self.logger.warning(f'input point_cloud is None!')
            return
    ...
    def get_descendent_leaves(self,
        graph,
        height,
        vertex
    ):
    ...
    def propagate_leaf_barcodes(self,
        graph,
        height,
        leaf_barcode
    ):
    ...
    def subdivide_edges_single_height(self,
        graph,
        height,
        subdiv_height
    ):
        ...
        for edge in graph.edges():
            if ((height[edge[0]] < subdiv_height and subdiv_height < height[edge[1]]) or 
    ...
    def get_heights_and_subdivide_edges(self,
        graph,
        height1,
        height2,
        mesh
    ...
    def merge_tree_interleaving_distance(self,
        merge_tree1, 
        merge_tree2, 
        mesh, 
        verbose = True, 
        ...
        if return_subdivided:
            Mgraph1_sub = MergeTreeModule(tree=graph1_sub, height=height1_sub, simplify=False)
            Mgraph2_sub = MergeTreeModule(tree=graph2_sub, height=height2_sub, simplify=False)

            return Mgraph1_sub, Mgraph2_sub, res

        else: return res

    def fusedGW_interleaving_decorated_trees(self,
        graph1_sub,
        height1_sub,
        node_barcode1,
        graph2_sub,
    ...
    def DMT_interleaving_distance(self,
        merge_tree1,
        merge_tree2, 
        mesh,
        thresh = 1e-5, 
    ...
    """
    def threshold_decorated_merge_tree(self,
        graph,
        height,
        leaf_barcode,
        thresh
    ...
    def draw_simplified_merge_tree(self,
        graph,
        heights,
        title = None, 
        figsize = (10,10), 
    ...
    def visualize_DMT_pointcloud(self,
        graph,
        height,
        barcode1,
        data,
        ...
        for edge in T_thresh.edges():  graph_DMT.add_edge(edge[0],edge[1],color = 'black', weight = 1)
        for edge in T_offsets.edges(): graph_DMT.add_edge(edge[0],edge[1],color = 'r', weight = 2)
        ...
        if draw:
            ...
            nx.draw_networkx(graph_DMT, pos = pos_DMT, edge_color=colors, width=weights,node_size = 0,with_labels = False)
            ax = plt.gca()
            ax.tick_params(left=True, bottom=False, labelleft=True, labelbottom=False)
