...
class BlipNetModule(GenericModule):
    ...
    def check_config(self):
        if self.mode == 'evaluate_merge_tree':
            if "merge_tree" not in self.config.keys():
                self.logger.error("'merge_tree' section not specified in config!")
        ...
        if self.mode == 'mcts':
            if "mcts" not in self.config.keys():
                self.logger.error("'mcts' section not specified in config!")
    ...
    def parse_blip_graph(self):
        ...
        if "blip_graph" not in merge_tree_config.keys():
            self.logger.error("blip_graph not specified in merge_tree config!")
    ...
    def run_evaluate_merge_tree(self):
        ...
        with torch.no_grad():
            for ii, data in inference_loop:
                ...
                # update progress bar
                if (self.config['merge_tree']['progress_bar'] is True):
                    inference_loop.set_description(
                        f"Merge Tree Inference: Event [{ii+1}/{len(self.meta['loader'].inference_loader)}]"
                    )
                    inference_loop.set_postfix_str(f"nodes={len(merge_tree['clusters'][0])}")

        # save all predictions
        np.savez(
            'blip_graph_outputs.npz',
            **predictions
        )
